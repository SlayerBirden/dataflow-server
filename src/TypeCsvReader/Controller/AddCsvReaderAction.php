...
final class AddCsvReaderAction implements MiddlewareInterface
{
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        try {
            $entity = new CsvReader();
            $this->hydrator->hydrate($data, $entity);
            $em = $this->managerRegistry->getManagerForClass(CsvReader::class);
            $em->persist($entity);
            $em->flush();
            $msg = 'CsvReader has been successfully created!';
            return (new ResponseFactory())($msg, 200, 'csv_reader', $this->hydrator->extract($entity));
        } catch (ORMInvalidArgumentException | ValidationException $exception) {
            return (new ResponseFactory())($exception->getMessage(), 400, 'csv_reader');
        } catch (ORMException $exception) {
            $this->logger->error((string)$exception);
            return (new ResponseFactory())('Error during creation operation.', 400, 'csv_reader');
        }
    }
}
